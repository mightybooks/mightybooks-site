...
export async function getAdminLibraryBookList({ status, q, page, pageSize, from, to }) {
  let query = supabaseAdmin
    .from('books')
    .select('id,slug,title,display_title,status,cover_image_path,display_order,updated_at', { count: 'exact' })
  ...
  if (q) {
    query = query.or(`title.ilike.%${q}%,display_title.ilike.%${q}%,slug.ilike.%${q}%`)
  }
