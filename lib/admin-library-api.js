...
export const ADMIN_LIBRARY_PAGE_SIZE = 30

const UUID_PATTERN =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
const AUTHOR_SLUG_PATTERN = /^[a-z0-9]+(-[a-z0-9]+)*$/
const SOCIAL_LINK_TYPES = new Set([
  'homepage', 'instagram', 'blog', 'youtube', 'facebook', 'threads', 'x', 'brunch', 'other',
])
...
export function parseAdminListParams(request, statuses) {
  const searchParams = new URL(request.url).searchParams
  const status = searchParams.get('status') ?? 'all'
  const q = (searchParams.get('q') ?? '').trim().slice(0, 100)
  const pageValue = Number(searchParams.get('page') ?? '1')
  const pageSizeValue = Number(
    searchParams.get('pageSize') ?? String(ADMIN_LIBRARY_PAGE_SIZE)
  )

  if (status !== 'all' && !statuses.includes(status)) {
    return { error: validationError('INVALID_STATUS', '올바른 상태를 선택해 주세요.') }
  }
  if (!Number.isInteger(pageValue) || pageValue < 1) {
    return { error: validationError('INVALID_PAGE', '페이지 번호를 확인해 주세요.') }
  }
  if (!Number.isInteger(pageSizeValue) || pageSizeValue < 1 || pageSizeValue > ADMIN_LIBRARY_PAGE_SIZE) {
    return { error: validationError('INVALID_PAGE_SIZE', '페이지당 항목 수는 1~30이어야 합니다.') }
  }
  ...
  return {
    value: {
      ...
      from: (pageValue - 1) * pageSizeValue,
      to: pageValue * pageSizeValue - 1,
    },
  }
}
...
function normalizeWebUrl(value, code, message) {
...
function normalizeOrderedLinks(value, type) {
  const code = type === 'social' ? 'INVALID_SOCIAL_LINKS' : 'INVALID_EXTERNAL_LINKS'
  const message = type === 'social'
    ? '홈페이지·SNS 채널 정보를 확인해 주세요.'
    : '외부 링크 배너 정보를 확인해 주세요.'
  if (!Array.isArray(value)) return validationError(code, message)
  const result = []
  for (const item of value) {
    if (!item || typeof item !== 'object' || Array.isArray(item) ||
      typeof item.is_visible !== 'boolean' || !Number.isInteger(item.sort_order) || item.sort_order < 0) {
      return validationError(code, message)
    }
    const url = normalizeWebUrl(item.url, code, message)
    if (url.error) return url
    if (type === 'social') {
      if (typeof item.type !== 'string' || !SOCIAL_LINK_TYPES.has(item.type)) {
        return validationError(code, message)
      }
      result.push({ type: item.type, url: url.value, is_visible: item.is_visible, sort_order: item.sort_order })
    } else {
      const title = normalizeOptionalString(item.title)
      const description = normalizeOptionalString(item.description)
      if (title.invalid || !title.value || title.value.length > 120 ||
        description.invalid || (description.value?.length ?? 0) > 300) {
        return validationError(code, message)
      }
      result.push({ title: title.value, description: description.value, url: url.value,
...
function normalizePressItems(value) {
  if (!Array.isArray(value)) return validationError('INVALID_PRESS_ITEMS', '언론 보도 정보를 확인해 주세요.')
  const result = []
  for (const item of value) {
    const outlet = normalizeOptionalString(item?.outlet_name)
    const title = normalizeOptionalString(item?.title)
    const summary = normalizeOptionalString(item?.summary)
    const url = normalizeWebUrl(item?.source_url, 'INVALID_PRESS_ITEMS', '언론 보도 원문 주소를 확인해 주세요.')
    const parsedDate = typeof item?.published_at === 'string'
      ? new Date(`${item.published_at}T00:00:00Z`)
      : null
    const validDate = /^\d{4}-\d{2}-\d{2}$/.test(item?.published_at ?? '') &&
      !Number.isNaN(parsedDate?.getTime()) &&
      parsedDate.toISOString().slice(0, 10) === item.published_at
    if (outlet.invalid || !outlet.value || outlet.value.length > 120 || title.invalid || !title.value ||
      title.value.length > 200 || summary.invalid || (summary.value?.length ?? 0) > 500 || url.error ||
      !validDate || typeof item.is_visible !== 'boolean' || !Number.isInteger(item.sort_order) || item.sort_order < 0) {
      return validationError('INVALID_PRESS_ITEMS', '언론 보도 정보를 확인해 주세요.')
    }
    result.push({ outlet_name: outlet.value, title: title.value, published_at: item.published_at,
      summary: summary.value, source_url: url.value, is_visible: item.is_visible, sort_order: item.sort_order })
  }
  return { value: result }
}
...
export function validateAuthorPayload(body) {
  ...
  const socialLinks = normalizeOrderedLinks(body.social_links ?? [], 'social')
  if (socialLinks.error) return socialLinks
  const externalLinks = normalizeOrderedLinks(body.external_links ?? [], 'external')
  if (externalLinks.error) return externalLinks
  if (body.press_enabled !== undefined && typeof body.press_enabled !== 'boolean') {
    return validationError('INVALID_PRESS_ENABLED', '언론 보도 페이지 사용 여부를 확인해 주세요.')
  }
  const pressItems = normalizePressItems(body.press_items ?? [])
  if (pressItems.error) return pressItems
  ...
  return {
    value: {
      ...
      press_enabled: body.press_enabled ?? false,
      press_items: pressItems.value,
    },
  }
}
...
export async function getAdminLibraryAuthor(authorId) {
  const { data: author, error: authorError } = await supabaseAdmin
    .from('authors')
    .select(
      'id,slug,display_name,status,pen_name,occupation,profile_image_path,short_bio,bio_paragraphs,display_order,social_links,external_links,press_enabled,created_at,updated_at'
    )
    .eq('id', authorId)
    .maybeSingle()
  ...
  const [sectionsResult, bookLinksResult, membershipsResult, pressResult] =
    await Promise.all([
      supabaseAdmin
      ...
      supabaseAdmin
        ...
        .select('id,outlet_name,title,published_at,summary,source_url,is_visible,sort_order,created_at,updated_at')
        .eq('author_id', authorId)
        .order('sort_order', { ascending: true })
