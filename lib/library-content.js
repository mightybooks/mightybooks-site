...
export const getPublishedLibraryAuthorRedirect = cache(async function getAuthorRedirect(oldSlug) {
  const { data, error } = await supabaseAdmin
...
export const getPublishedAuthorPressPage = cache(async function getAuthorPressPage(authorSlug) {
  const { data: author, error: authorError } = await supabaseAdmin
    .from('authors')
    .select('id,slug,display_name,press_enabled')
    .eq('slug', authorSlug)
    .eq('status', 'published')
    .maybeSingle()
  ...
  return { author: { slug: author.slug, displayName: author.display_name }, items }
})
...
export async function getPublishedLibrarySitemapEntries() {
