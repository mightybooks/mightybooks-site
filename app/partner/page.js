...
const steps = ['로그인 또는 계정 생성','업체 정보 입력','사업자등록증 사본과 온라인 활동 정보 제출','마이티북스 검토','승인 후 고유 파트너 코드 발급','전용 링크와 홍보자료 제공']
...
export default function PartnerPage(){return <main className={styles.wrap}>
  ...
  <section className={styles.sectionAlt}><div className={styles.inner}><span className={styles.eyebrow}>Partner Support</span><h2 className={styles.title}>승인 파트너에게 제공하는 <em>지원</em></h2><ul className={styles.list}>{['업체별 고유 파트너 코드와 영구 추천 링크','전용 QR과 모바일 전달용 홍보 이미지','고객용 안내 카드','필요 시 매장용 미니 배너','소개 실적 및 정산 안내 - 실시간으로 파트너 페이지 대시보드에서 확인 가능합니다'].map(item=><li key={item}>{item}</li>)}</ul><div className={styles.notice}>오프라인 홍보물은 모든 가입자에게 자동 제공되지 않습니다. 업체의 운영 환경과 활용 방식을 확인한 뒤 협의하여 제작합니다.</div></div></section>

  <section className={styles.section}><div className={styles.inner}><span className={styles.eyebrow}>Application Process</span><h2 className={styles.title}>신청과 <em>승인 절차</em></h2><div className={styles.steps}>{steps.map((step,index)=><article key={step} className={styles.step}><span>{String(index+1).padStart(2,'0')}</span><h3>{step}</h3></article>)}</div><div className={styles.notice}><strong>사업자등록증은 홈페이지에 업로드하지 않습니다.</strong><br/>신청 후 사업자등록증 사본과 업체 홈페이지 또는 SNS, 네이버 플레이스·블로그·카카오채널 등 실제 운영을 확인할 수 있는 주소를 <a href="mailto:novelstudylab@naver.com">novelstudylab@naver.com</a>으로 보내 주세요.<br/><br/>메일 제목: [마이티북스 제휴 신청] 업체명 / 가입 이메일<br/><br/>파트너십 신청은 자동 승인되지 않습니다. 마이티북스가 실제 운영 여부, 고객층과 서비스 적합성을 검토한 뒤 승인합니다.</div><div className={styles.actions}><Link href="/partner/signup" className={styles.primary}>파트너십 신청하기</Link><Link href="/partner/login" className={styles.secondary}>파트너 로그인</Link></div></div></section>
