...
export default function PartnerPage(){return <main className={styles.wrap}>
  ...
  <section className={styles.sectionAlt}><div className={styles.inner}><span className={styles.eyebrow}>Business Partners</span><h2 className={styles.title}>고객의 중요한 순간을 함께 준비하는 <em>사업자 파트너십</em></h2><p className={styles.sectionLead}>환갑, 칠순, 팔순, 퇴임과 가족 기념일을 준비하는 꽃집, 떡·답례품 업체, 케이크 제작업체와 가족행사 관련 사업자를 우선적으로 찾고 있습니다. 이 업종에 한정하지 않으며 고객에게 책 제작이라는 선택지를 자연스럽게 안내할 수 있는 사업자라면 신청할 수 있습니다.</p><div className={styles.grid}><article className={styles.card}><h3>고객에게 제공되는 혜택</h3><p>확인된 파트너 소개로 처음 문의한 신규 고객에게 제작비 할인과 계약 조건에 따른 혜택을 안내합니다.</p></article><article className={styles.card}><h3>승인 파트너 지원</h3><p>업체별 고유 코드, 영구 추천 링크, 전용 QR과 모바일 홍보 이미지를 순차적으로 제공합니다.</p></article><article className={styles.card}><h3>계약 성사 시 소개료</h3><p>소개를 통해 신규 고객의 제작 계약이 성사되면 파트너 업체에 소개료를 지급하며 세부 조건은 승인 후 별도로 안내합니다.</p></article></div></div></section>

  <section className={styles.section}><div className={styles.inner}><span className={styles.eyebrow}>Simple Role</span><h2 className={styles.title}>파트너가 하는 일은 <em>간단합니다</em></h2><p className={styles.sectionLead}>책 제작이 필요한 고객에게 마이티북스의 제휴 혜택을 안내해 주세요. 상담, 견적, 계약, 제작과 고객 응대는 마이티북스가 직접 담당하며 파트너가 책 제작을 대신 판매하거나 상담할 필요는 없습니다.</p><div className={styles.grid}>{['전용 링크 전달','QR 안내','고객용 카드 전달','마이티북스 소개'].map(item=><article key={item} className={styles.card}><h3>{item}</h3></article>)}</div></div></section>

  <section className={styles.sectionAlt}><div className={styles.inner}><span className={styles.eyebrow}>Partner Support</span><h2 className={styles.title}>승인 파트너에게 제공하는 <em>지원</em></h2><ul className={styles.list}>{['업체별 고유 파트너 코드와 영구 추천 링크','전용 QR과 모바일 전달용 홍보 이미지','고객용 안내 카드','필요 시 매장용 미니 배너','소개 실적 및 정산 안내'].map(item=><li key={item}>{item}</li>)}</ul><div className={styles.notice}>오프라인 홍보물은 모든 가입자에게 자동 제공되지 않습니다. 업체의 운영 환경과 활용 방식을 확인한 뒤 협의하여 제작합니다.</div></div></section>
