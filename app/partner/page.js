...
export default function PartnerPage(){return <main className={styles.wrap}>
  ...
  <section className={styles.section}><div className={styles.inner}><div className={`${featureStyles.featureIntro} ${featureStyles.featureIntroReverse}`}><div><span className={styles.eyebrow}>Simple Role</span><h2 className={styles.title}>파트너가 하는 일은 <em>간단합니다</em></h2><p className={styles.sectionLead}>책 제작이 필요한 고객에게 마이티북스의 제휴 혜택을 안내해 주세요. 상담, 견적, 계약, 제작과 고객 응대는 마이티북스가 직접 담당하며 파트너가 책 제작을 대신 판매하거나 상담할 필요는 없습니다.</p></div><PartnerImage src="/image/home/partner01.webp" alt="파트너 업체가 고객에게 마이티북스 출판 서비스를 간단히 안내하는 과정"/></div><div className={styles.grid}>{['전용 링크 전달','QR 안내','고객용 카드 전달','마이티북스 소개'].map(item=><article key={item} className={styles.card}><h3>{item}</h3></article>)}</div></div></section>
