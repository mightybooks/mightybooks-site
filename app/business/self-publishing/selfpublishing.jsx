...
const professionalLibraryUses = [
  '전문서적 전체 또는 일부를 고품질 플립북으로 제공',
  '저자 약력, 전문 분야와 출간 도서를 한 공간에서 소개',
  '강의, 교육, 연구, 컨설팅 활동과 출간 도서를 연결',
  '학교, 기관, 협회, 고객에게 링크로 간편하게 공유',
  '출간 도서를 전문성을 입증하는 온라인 포트폴리오로 활용',
  '여러 권의 저서를 하나의 개인 온라인 서가에 축적',
...
export default function SelfPublishingPage() {
  ...
  return <main className={styles.wrap}>
    ...
    <section className={styles.section} ref={reveal(13)} aria-label="전문성을 증명하는 온라인 서가"><SectionHeader eyebrow="Author Library" title={<>출간 도서를 전문성을 증명하는<br /><em>온라인 서가로</em></>}><>종이책 출간에서 끝나는 것이 아니라, 저자의 지식과 경력을 온라인에서 지속적으로 확인할 수 있는 형태로 확장합니다.</></SectionHeader><div className={styles.useGrid}>{professionalLibraryUses.map((item,index) => <article key={item}><span>{String(index+1).padStart(2,'0')}</span><h3>{item}</h3></article>)}</div><div className={styles.compare}><p>전문서적은 저자의 지식과 경험을 정리한 결과물이자 전문성을 보여주는 중요한 자료입니다. 고품질 플립북과 개인 온라인 서가를 활용하면 출간 도서를 강의, 교육, 연구, 컨설팅, 기관 제안 등 다양한 활동과 연결할 수 있습니다.</p><p>책 전체 또는 공개 가능한 일부 내용을 온라인에서 열람하도록 구성하고, 저자 약력과 전문 분야, 출간 도서 정보를 한 공간에 정리할 수 있습니다.</p></div><p className={styles.notice} style={{ textAlign: 'center' }}>전문서적을 단순한 출간 결과물이 아니라, 저자의 지식과 경력을 지속적으로 축적하는 전문 브랜드 자산으로 확장합니다.</p></section>
    <section className={styles.sectionAlt} ref={reveal(14)} aria-labelledby="royalty-heading"><SectionHeader eyebrow="Royalty by Sales Channel" title={<>판매 채널에 따른 <em id="royalty-heading">인세 안내</em></>}>완성된 도서는 일반 서점 유통과 마이티북스 온라인 서가를 통해 판매할 수 있습니다. 판매 채널에 따라 저자에게 지급되는 인세율이 다르게 적용됩니다.</SectionHeader><div className={styles.categoryGrid}><article className={styles.categoryCard}><span>45%</span><h3>서점 유통 · 인세 45% 지급</h3><p>서점과 유통사를 통한 판매 시 정가를 기준으로 저자 인세 45%를 지급합니다.</p></article><article className={styles.categoryCard}><span>80%</span><h3>온라인 서가 · 인세 80% 지급</h3><p>마이티북스 온라인 서가에서 직접 판매할 경우 저자 인세 80%를 지급합니다.</p></article></div></section>
