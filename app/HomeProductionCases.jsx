...
import BookPreviewModalRouter from './business/components/book-preview/BookPreviewModalRouter'
import {
...
const cases = [
  ...
  { type: '전문서·실용서', title: '실제 제작 전문·실용도서', process: '구성 검토 · 편집 · 디자인 · 정식 출판', book: selfPublishingPreview },
    ...
    <>
      <div className={styles.bookGrid}>
        {cases.map((item) => {
          ...
          const content = <><div className={styles.bookImg}><Image src={image} alt={`${item.title} 표지`} fill sizes="260px" /><span className={styles.bookNum}>{item.type}</span></div><div className={styles.bookInfo}><div className={styles.bookGenre}>{item.type}</div><h3 className={styles.bookTitle}>{item.title}</h3><p className={styles.bookAuthor}>{item.process}</p>{item.book && <span className={styles.previewLink}>{item.book.highResolution ? '고화질' : '3D 표지'}·{item.book.pages.length}페이지 내지 보기 →</span>}{item.externalHref && <span className={styles.previewLink}>수림지 웹북 바로 보기 →</span>}</div></>
      ...
      {selectedBook && <BookPreviewModalRouter book={selectedBook} onClose={closePreview} />}
    </>
  )
}
